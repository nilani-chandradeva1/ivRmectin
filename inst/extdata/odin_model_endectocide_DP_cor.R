...
DP_scaler <- user() #between 0 and 1, used to explore potential correlations with a prohylactic e.g. DP, amongst ivm_fed mosq
FOIv <- delay(lag_FOIv, delayGam)
FOIv_IVM <- FOIv*DP_scaler
...
deriv(Sx_F1[1:eff_len]) = if (t >= (IVRM_sr + i -1)  &&  t < (IVRM_sr + i) && t < (IVRM_sr + eff_len )) ivm_cov*(avhc-FOIv_IVM)*Sv - avhc*Sx_F1[i] - mu_vi[i]*Sx_F1[i]     else - mu_vi[i]*Sx_F1[i] - avhc*Sx_F1[i]

deriv(Sx_F2[1:eff_len]) = if (t >= (IVRM_sr + i -1)  &&  t < (IVRM_sr + i) && t < (IVRM_sr + eff_len )) ivm_cov*(avhc-FOIv_IVM)*(Sv_F1 + Sv_F2) + (avhc - FOIv_IVM)*Sx_F1[i] - FOIv_IVM*Sx_F2[i] - mu_vi[i]*Sx_F2[i]     else  (avhc- FOIv_IVM)*Sx_F1[i] - mu_vi[i]*Sx_F2[i] - FOIv_IVM*Sx_F2[i]
...
deriv(Ex_F1[1, 1:eff_len]) =            if (t >= (IVRM_sr + j -1)  &&  t < (IVRM_sr + j ) && t < (IVRM_sr + eff_len)) FOIv_IVM*ivm_cov*Sv  - 1/tau_v*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j] - avhc*Ex_F1[i,j] else - 1/tau_v*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j]  - avhc*Ex_F1[i,j]
...
deriv(Ex_F2[1, 1:eff_len]) =            if (t >= (IVRM_sr + j -1)  &&  t < (IVRM_sr + j ) && t < (IVRM_sr + eff_len)) ivm_cov*FOIv_IVM*(Sv_F1 + Sv_F2) + FOIv_IVM*(Sx_F1[j] + Sx_F2[j]) + avhc*ivm_cov*(Ev_F1[i] + Ev_F2[i]) + avhc*Ex_F1[i,j] - 1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_F2[i,j] else  avhc*Ex_F1[i,j] + FOIv_IVM*(Sx_F1[j] + Sx_F2[j]) - 1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_F2[i,j]
