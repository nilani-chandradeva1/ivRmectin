...
my_sim_ivm_mda_mod <- function(){
  ...
  out_df_antag <- do.call(rbind, sapply(1:(nrow(ivm_params_df)), function(x){
    ...
    df3 <- as.data.frame(dplyr::mutate(.data = df2, ref = x, scenario = "ivm_placebo_int_arm"))}, simplify = F))
  return(out_df_antag)
...
my_sim_ivm_mda_mod_control <- function(){
  ...
  out_df_antag <- do.call(rbind, sapply(1:(nrow(ivm_params_df)), function(x){
    ...
    df2 <- as.data.frame(dplyr::select(.data = df,t, mu, mv, avhc, itn_cov, EIR_tot, slide_prev0to5, slide_prev0to80,
                                       Q0, IVRM_sr, EIRout, clin_inc0to5, bites_Bed, Ivtot))
    df3 <- as.data.frame(dplyr::mutate(.data = df2, ref = x, scenario = "ivm_placebo_control_arm"))}, simplify = F))
  return(out_df_antag)
...
placebo_sens_df <- rbind(my_sim_ivm_mda_out, my_sim_ivm_mda_out_control)
saveRDS(placebo_sens_df, file = "analysis/matamal-runs/placebo_sens_df.rds")
...
my_sim_ivm_mda_out %>%
  ...
  select(slide_prev0to80) #~59.8% control arm prevalence

my_sim_ivm_mda_out_control %>%
  filter(t == prev_survey_date_2022) %>%
  select(slide_prev0to80) #81.6% prevalence
